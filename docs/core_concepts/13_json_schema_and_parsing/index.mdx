...
There are special types that lead to a custom behavior in the [auto-generated UIs](../6_auto_generated_uis/index.mdx).
...
Windmill offers robust backend schema validation to ensure that script arguments strictly conform to their defined JSON Schema. While auto-generated UIs provide client-side checks, this server-side validation is essential for programmatic invocations, such as through [webhooks](../4_webhooks/index.mdx) or direct API calls, where input data might bypass UI-level validation. It prevents scripts from running with malformed or incorrect data.

To enforce schema validation for a script, add the `schema_validation` annotation (as a comment) at the beginning of your script file. When enabled, Windmill will reject job executions if the provided arguments do not match the schema. The job logs will indicate whether schema validation is active and report any validation errors. This validation acts as a crucial safeguard, ensuring data integrity and preventing unexpected runtime errors, thereby enhancing the overall reliability of your workflows.
